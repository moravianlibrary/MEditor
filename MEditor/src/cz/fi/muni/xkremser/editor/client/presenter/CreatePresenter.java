 ...
 */
public class CreatePresenter extends Presenter<CreatePresenter.MyView, CreatePresenter.MyProxy> implements MyUiHandlers {
	...
	public interface MyProxy extends ProxyPlace<CreatePresenter> {
	 ...
	 */
	@Inject
	public CreatePresenter(final EventBus eventBus, final MyView view, final MyProxy proxy, final DigitalObjectMenuPresenter leftPresenter,
	...
	public void prepareFromRequest(PlaceRequest request) {
		...
		uuid = request.getParameter(Constants.URL_PARAM_CODE, null);
	...
	public void onAddDigitalObject(final TileGrid tileGrid, final Menu menu) {
		...
		items[0].addClickHandler(new com.smartgwt.client.widgets.menu.events.ClickHandler() {
			...
			public void onClick(MenuItemClickEvent event) {
				String uuidToEdit = tileGrid.getSelection()[0].getAttribute(Constants.URL_PARAM_CODE);
				placeManager.revealRelativePlace(new PlaceRequest(NameTokens.MODIFY).with(Constants.URL_PARAM_CODE, uuidToEdit));
			}
		});
	...
	public void onAddDigitalObject(final String uuid, final ImgButton closeButton, final Menu menu) {
		closeButton.addClickHandler(new ClickHandler() {
			...
			public void onClick(ClickEvent event) {
				DigitalObjectClosedEvent.fire(CreatePresenter.this, uuid);
			}
		});
	}
		...
		final DispatchCallback<GetDigitalObjectDetailResult> callback = new DispatchCallback<GetDigitalObjectDetailResult>() {
			...
			public void callback(GetDigitalObjectDetailResult result) {
				...
				DigitalObjectOpenedEvent
						.fire(CreatePresenter.this, true, new RecentlyModifiedItem(uuid, title, "", detail.getModel()), result.getDetail().getRelated());
				getView().getPopupPanel().setVisible(false);
				getView().getPopupPanel().hide();
			}
